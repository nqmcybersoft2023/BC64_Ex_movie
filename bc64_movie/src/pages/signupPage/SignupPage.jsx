...
const SignUpPage = () => {
    const formSignup = useFormik({
        ...
        onSubmit: async (value) => {
            try {
                const data = await userSer.postSignup(value);
    ...
    return (
        <div>
            <form className='border p-3 rounded-md space-y-3' onSubmit={formSignup.handleSubmit}>
                <div>
                    {/* TÀI KHOẢN  */}
                    <h2 className='text-2xl font-medium flex justify-center'>Đăng ký</h2>
                    <label className='text-sm font-medium ' htmlFor="taiKhoan">Tài khoản</label>
                    <input
                        id="taiKhoan"
                        name="taiKhoan"
                        type="text"
                ...
                {/* NHẬP LẠI PASS  */}
                <div>
                    <label className="text-sm font-medium" htmlFor="nhapLaiMatKhau">Nhập lại mật khẩu</label>
                    <input
                        id="nhapLaiMatKhau"
                        name="nhapLaiMatKhau"
                        type="password"
                ...
                <div>
                    <label className="text-sm font-medium" htmlFor="hoTen">Họ tên</label>
                    <input
                        id="hoTen"
                        name="hoTen"
                        type="text"
